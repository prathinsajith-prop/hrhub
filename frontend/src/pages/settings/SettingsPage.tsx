...
// ─── Mock data ────────────────────────────────────────────────────────────────
const mockUsers = [
    { id: '1', name: 'Ahmed Al Mansouri', email: 'ahmed@hrhub.ae', role: 'Super Admin', status: 'active', lastLogin: '2 hours ago' },
    { id: '2', name: 'Fatima Al Zaabi', email: 'fatima@hrhub.ae', role: 'HR Manager', status: 'active', lastLogin: '1 day ago' },
    { id: '3', name: 'John Smith', email: 'john@hrhub.ae', role: 'Payroll Officer', status: 'active', lastLogin: '3 days ago' },
    { id: '4', name: 'Sara Hassan', email: 'sara@hrhub.ae', role: 'PRO Officer', status: 'inactive', lastLogin: '2 weeks ago' },
]
...
const notifGroups = [
    {
        ...
        items: [
            { id: 'visa_expiry', label: 'Visa expiry reminders', desc: 'Alerts 30, 14, 7 days before expiry', email: true, sms: false, push: true },
            { id: 'eid_expiry', label: 'Emirates ID expiry', desc: 'Alerts before EID expires', email: true, sms: false, push: true },
            { id: 'doc_missing', label: 'Missing documents', desc: 'Notify when employee docs are incomplete', email: true, sms: false, push: false },
        ],
    },
    {
        title: 'Leave & Attendance',
        items: [
            { id: 'leave_request', label: 'New leave request', desc: 'When employees submit leave requests', email: true, sms: false, push: true },
            { id: 'leave_approved', label: 'Leave approved/rejected', desc: 'Notify employee of decision', email: true, sms: true, push: true },
        ],
    },
    {
        title: 'Payroll',
        items: [
            { id: 'payroll_ready', label: 'Payroll run ready for approval', desc: 'Monthly payroll notification', email: true, sms: false, push: true },
            { id: 'wps_submitted', label: 'WPS submission confirmation', desc: 'After successful WPS upload to MOHRE', email: true, sms: true, push: false },
        ],
    },
]
    ...
    return (
        <div className="space-y-6">
                ...
                <CardContent className="space-y-4">
                    <div className="flex items-center gap-4 p-4 rounded-xl bg-muted/40 border">
                        <div className="h-16 w-16 rounded-2xl bg-primary flex items-center justify-center text-white text-xl font-bold">
                            {(tenant?.name ?? 'HR').slice(0, 2).toUpperCase()}
                        </div>
                        <div>
                            <p className="font-semibold">{tenant?.name ?? 'HRHub Demo Company'}</p>
                            <p className="text-sm text-muted-foreground">UAE Mainland · Dubai</p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                            ...
                            <Input id="company_name" defaultValue={tenant?.name ?? 'HRHub Demo Company'} />
                        </div>
                        <div className="space-y-1.5">
                            <Label htmlFor="trade_license">Trade License No.</Label>
                            <Input id="trade_license" defaultValue="DED-2021-12345" />
                        </div>
                        <div className="space-y-1.5">
                            <Label htmlFor="mohre_number">MOHRE Employer No.</Label>
                            <Input id="mohre_number" defaultValue="MOH-00456789" />
                        </div>
                        <div className="space-y-1.5">
                            <Label htmlFor="wps_id">WPS Employer ID</Label>
                            <Input id="wps_id" defaultValue="WPS-2021-9876" />
                        </div>
                        <div className="space-y-1.5">
                            <Label htmlFor="contact_email">Contact Email</Label>
                            <div className="relative">
                                <Mail className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                                <Input id="contact_email" className="pl-9" defaultValue="hr@company.ae" />
                            </div>
                        </div>
                        <div className="space-y-1.5">
                            ...
                            <div className="relative">
                                ...
                                <Input id="contact_phone" className="pl-9" defaultValue="+971 4 123 4567" />
                            </div>
                        </div>
                        <div className="space-y-1.5 md:col-span-2">
                            ...
                            <div className="relative">
                                ...
                                <Input id="address" className="pl-9" defaultValue="Office 2301, Business Bay, Dubai, UAE" />
                            </div>
                        </div>
                    </div>
                </CardContent>
            ...
            <div className="flex justify-end">
                <Button onClick={handleSave} leftIcon={saved ? <CheckCircle2 className="h-4 w-4" /> : <Save className="h-4 w-4" />} variant={saved ? 'success' : 'default'}>
                        ...
                        {mockUsers.map((u) => (
                            <div key={u.id} className="flex items-center justify-between px-6 py-3.5 hover:bg-muted/30 transition-colors">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className={cn('text-xs font-medium px-2 py-0.5 rounded-full', roles.find(r => r.label.toLowerCase().replace(' ', '_') === u.role.toLowerCase().replace(' ', '_'))?.color ?? 'bg-gray-50 text-gray-600')}>
                                        {u.role}
                                    </span>
...
function NotificationsTab() {
    const [settings, setSettings] = useState(() =>
        notifGroups.flatMap(g => g.items).reduce((acc, item) => ({
            ...acc,
            [`${item.id}_email`]: item.email,
            [`${item.id}_sms`]: item.sms,
            [`${item.id}_push`]: item.push,
        ...
        <div className="space-y-6">
            ...
            <Card>
                ...
                <CardContent className="divide-y">
                    {[
                        { id: 'mfa', label: 'Two-Factor Authentication', desc: 'Require 2FA for all admin accounts', defaultChecked: false },
                        { id: 'session_timeout', label: 'Auto Session Timeout', desc: 'Log out after 30 minutes of inactivity', defaultChecked: true },
                        { id: 'ip_whitelist', label: 'IP Allowlist', desc: 'Restrict logins to specific IP ranges', defaultChecked: false },
