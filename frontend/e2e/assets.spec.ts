/**
 * Assets page tests — table renders, search works, pagination is correct.
 */
...
test.describe('Assets page', () => {
    ...
    test('shows heading', async ({ page }) => {
        await page.goto('/assets')
        await page.waitForLoadState('networkidle')
        await expect(page.getByRole('heading', { name: /asset/i }).first()).toBeVisible({ timeout: 10_000 })
    })
    ...
    test('only one pagination bar visible (no DataTable duplicate)', async ({ page }) => {
        ...
        expect(count, 'Duplicate pagination detected — DataTable internal pagination should be hidden').toBeLessThanOrEqual(1)
    })

    test('DataTable internal page controls hidden when used with server-side pagination', async ({ page }) => {
        await page.goto('/assets')
        await page.waitForLoadState('networkidle')
        const pageControls = page.locator('text=/Page \\d+ of \\d+/')
        const count = await pageControls.count()
        expect(count, '"Page X of Y" should not appear alongside TablePagination').toBe(0)
    })
    ...
    test('search filters results', async ({ page }) => {
        ...
        const emptyMsg = await page.getByText(/no assets|no results/i).isVisible().catch(() => false)
