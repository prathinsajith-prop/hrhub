...
// Tenant-configurable category taxonomy (great_work, helping_hand, innovation, ...).
// Seeded with a default set on first access. Categories are referenced by `key`
// (slug) on recognitions, so renaming the label preserves history.
...
// ── Recognition Badges ───────────────────────────────────────────────────────
// Configurable achievement badges. Level is a free-text grade (bronze/silver/
// gold/platinum, or whatever the tenant prefers). category_key links to the
// category taxonomy but is not enforced (FK by key, validated in service).
...
// ── Recognitions ─────────────────────────────────────────────────────────────
// The appreciation post. Recipients live in three M:N tables (employees, teams,
// dept). Visibility controls feed filtering ('public', 'team', 'department',
// 'branch', 'manager', 'hr', 'private'). Status tracks workflow lifecycle
...
// Many-to-many: an individual recognition can name N employees as recipients.
// isPrimary marks the lead recipient when multiple are listed (used in feed
// "X gave recognition to Y and 3 others" copy).
...
// ── Reactions ────────────────────────────────────────────────────────────────
// One reaction per (recognition, user). To change reaction, UPDATE in place.
...
// ── Comments ─────────────────────────────────────────────────────────────────
// Threaded via parent_id (single level of nesting in UI; storage is recursive).
// Soft-deleted via deleted_at; deleted_by_user_id distinguishes self-delete
// from moderator delete in the audit trail.
...
// ── Points ledger ────────────────────────────────────────────────────────────
// Append-only ledger. `type` discriminates: 'earned' (received from recognition),
// 'given' (deducted from giver's monthly allowance), 'granted' (admin top-up),
// 'redeemed' (cashed out). balance_after is a denormalised snapshot for fast
// "current balance" reads without aggregating the whole ledger.
...
// ── Approvals trail ──────────────────────────────────────────────────────────
// One row per approval action. The latest row's action determines the
// recognition's workflow state. step distinguishes manager_review from hr_approval.
