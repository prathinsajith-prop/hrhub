...
export const announcements = pgTable('announcements', {
    ...
    // Distinguishes official HR announcements from employee social posts.
    // 'post' = created via the employee portal (empty title, company-wide);
    // 'announcement' = authored by HR in the admin app. Drives the portal's
    // two-tab split (Announcements vs Posts) and per-card labelling.
...
// Threaded comments on an announcement. Mirrors recognition_comments.
// Visibility is enforced at the route layer (caller must clear the
// audience gate). Soft-delete via `deletedAt`; one level of threading
// via `parentId` (optional, UI is flat today).
