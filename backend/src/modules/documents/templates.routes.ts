...
export async function templateRoutes(fastify: any): Promise<void> {
    ...
    fastify.get('/templates', auth, async (request: any, reply: any) => {
        const data = await getTemplates(request.user.tenantId)
        return reply.send({ data })
    })

    // GET /api/v1/documents/templates/:id
    fastify.get('/templates/:id', auth, async (request: any, reply: any) => {
    ...
    fastify.post('/templates', adminAuth, async (request: any, reply: any) => {
    ...
    fastify.patch('/templates/:id', adminAuth, async (request: any, reply: any) => {
    ...
    fastify.delete('/templates/:id', adminAuth, async (request: any, reply: any) => {
        const row = await deleteTemplate(request.user.tenantId, request.params.id)
        if (!row) return reply.code(404).send({ error: 'Template not found' })
        return reply.code(204).send()
    })

    // POST /api/v1/documents/templates/:id/render — render template with variables
    fastify.post('/templates/:id/render', auth, async (request: any, reply: any) => {
    ...
    fastify.get('/:id/versions', auth, async (request: any, reply: any) => {
