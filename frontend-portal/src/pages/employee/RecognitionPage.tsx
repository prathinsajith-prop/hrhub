...
import { Trophy, Award, Plus, Loader2, MessageCircle, Send, Pin, X } from 'lucide-react'
...
const VISIBILITIES = [
    ...
    { value: 'private', label: 'Just the recipient' },
]
        ...
        <Card className={cn(r.isPinned && 'ring-1 ring-primary/20')}>
            <CardContent className="p-4 space-y-3">
                ...
                <div className="flex items-center justify-between gap-2 text-[11px] text-muted-foreground">
                    <span>{r.giverName ? `${t0('From')} ${r.giverName}` : ''} · {fmtDate(r.publishedAt ?? r.createdAt)}</span>
                </div>
...
// tiny non-i18n helper to keep the meta line terse
                    ...
                    <div className="space-y-1.5">
                        ...
                        <div className="max-h-44 overflow-auto rounded-lg border border-border divide-y divide-border">
                            {colleagues.isLoading ? (
                                <div className="p-3 text-sm text-muted-foreground"><Loader2 className="size-4 animate-spin" /></div>
                            ) : filtered.length === 0 ? (
                                ...
                                return (
                                    <button key={c.id} type="button" onClick={() => toggle(c.id)}
                                        ...
                                        <span className="min-w-0 flex-1">
                                            ...
                                            {c.designation && <span className="block truncate text-[11px] text-muted-foreground">{c.designation}</span>}
                                        </span>
                                        <span className={cn('size-4 rounded border', checked ? 'border-primary bg-primary' : 'border-muted-foreground/40')} />
                                    </button>
                                )
                            })}
                        </div>
                        {recipientIds.length > 0 && <p className="text-[11px] text-muted-foreground">{recipientIds.length} {t('recognition.selected', { defaultValue: 'selected' })}</p>}
                    </div>
    ...
    return (
        <Dialog open onOpenChange={(o) => { if (!o) onClose() }}>
            <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-auto">
                    ...
                    <>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex items-center gap-2 text-[11px] text-muted-foreground">
                                {r.giverName && <span>From {r.giverName}</span>}
                                <span>· {fmtDate(r.publishedAt ?? r.createdAt)}</span>
                                {r.points > 0 && <span className="inline-flex items-center gap-1 text-amber-600">· <Award className="size-3" /> {r.points} pts</span>}
                            </div>
                                ...
                                {(comments ?? []).filter((c) => !c.deletedAt).map((c) => (
                                    <div key={c.id} className="flex gap-2.5">
                                        ...
                                        <div className="min-w-0 flex-1">
                                            <p className="text-xs"><span className="font-medium">{c.authorName ?? 'Someone'}</span> <span className="text-muted-foreground">· {fmtDate(c.createdAt)}</span></p>
                ...
                <button type="button" onClick={onClose} className="absolute right-4 top-4 text-muted-foreground hover:text-foreground"><X className="size-4" /></button>
            </DialogContent>
        </Dialog>
    )
}
