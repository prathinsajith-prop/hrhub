    ...
    return (
        <div className="space-y-5">
            ...
            {isLoading ? (
                ...
                <EmptyState icon={<Bell className="h-8 w-8" />} title={t('notifications.empty')} />
            ) : (
                        ...
                        const card = (
                            <Card className={cn('border-border/70 transition-colors', !n.isRead && 'border-primary/40 bg-primary/5')}>
                                <CardContent className="flex items-start gap-3 p-4">
                                    <span className={cn('mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-full', tone.tone)}>
                                        {tone.node}
                                    </span>
