 ...
 * insert their own spinner buttons or accept characters like `e` / `+`. We
 * sanitize keystrokes ourselves and surface a normal `change` event with the
 * cleaned string, so existing call sites that read `e.target.value` keep
 * working.
 *
 * Mirrors frontend/src/components/ui/numeric-input.tsx — kept as a separate
 * copy because the portal is a different workspace with no alias to the admin
 * frontend. If either side changes, update both.
 */
